...
namespace Willbound.Engine
{
    public sealed class MatchRunner
    {
        ...
        string StoreAction(Player player, PlayerAction action)
        {
            ...
            if (kind == StoreActionKind.Buy && action.StoreSlotIndex.HasValue)
            {
                var slot = action.StoreSlotIndex.Value;
                if (slot >= 0 && slot < Match.Store.Length && Match.Store[slot] != null)
                {
                    var buyCost = Match.Store[slot].Printing.StoreWorth;
                    if (player.Worth < buyCost)
                        return "Insufficient Worth.";
                    player.Worth -= buyCost;
                    paidWorth = buyCost;
                    Emit(EventKind.WorthChanged, "player", player.Id, "delta", -buyCost);
                }
            }
