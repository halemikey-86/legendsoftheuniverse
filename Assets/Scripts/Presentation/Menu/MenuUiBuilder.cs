...
namespace LegendsOfTheUniverse.Presentation.Menu
{
    public static class MenuUiBuilder
    {
        ...
        public static Font DefaultFont => GameFonts.Default;
        ...
        public static Image CreateTitleSprite(Transform parent, Sprite sprite, Vector2 size)
        {
            var titleObject = CreateRect("TitleSprite", parent);
            var rect = titleObject.GetComponent<RectTransform>();
            rect.anchorMin = new Vector2(0.5f, 0.82f);
            rect.anchorMax = new Vector2(0.5f, 0.82f);
            rect.sizeDelta = size;

            var image = titleObject.gameObject.AddComponent<Image>();
            image.sprite = sprite;
