...
namespace LegendsOfTheUniverse.Presentation
{
    ...
    public class ZoneView : MonoBehaviour
    {
        ...
        [Header("Pile")]
        [SerializeField] Quaternion pileStackRotation = Quaternion.identity;

        [Header("Label")]
        [SerializeField] bool showMarkers;
        [SerializeField] Vector3 labelWorldOffset = Vector3.zero;
        [SerializeField] Vector2 labelPanelSize = new(72f, 40f);
        ...
        public void ConfigurePile(string label, Vector3 worldPosition, Quaternion? stackRotation = null)
        {
            zoneLabel = label;
            zoneKind = ZoneKind.Pile;
            slotCount = 1;
            transform.position = worldPosition;
            if (stackRotation.HasValue)
                pileStackRotation = stackRotation.Value;
            BuildZone();
        }
        ...
        {
            pileStackRotation = rotation;
            EnsureRoots();
            if (cardsRoot != null)
                cardsRoot.localRotation = pileStackRotation;
        }
        ...
        void ApplyPileCardOrientation(CardView card)
        {
            if (pileStackRotation != Quaternion.identity)
                card.ApplyStackOrientation();
            else
                card.ApplyTableOrientation();
        }
